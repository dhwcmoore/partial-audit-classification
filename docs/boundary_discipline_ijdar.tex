...
% NOTE: the detailed overlap justification (which sections/definitions
% correspond to which manuscript, why this division is not duplicate
% publication) belongs in the cover letter to each editor, not here --
% keep this paragraph to a concise disclosure. Record the actual
% notification date below once sent; do not leave language implying
% it has already happened before it has.
\medskip
\noindent\textbf{Note on a related submission:} A companion manuscript, \emph{A Discipline of Partial Audit Classification: On the Correct Treatment of Undefined Audit Assertions}, is being prepared for submission to the \emph{International Journal on Software Tools for Technology Transfer}. Both manuscripts develop the same central invariant (Principle~\ref{prin:invariant}) and share the Coq/Rocq development described above; the present manuscript's central contribution is the accounting information systems architecture, and the companion manuscript's is the executable kernel and its soundness proofs. The cover letter to the editor gives the full division of content and overlap assessment. The corresponding author will notify the editors of both journals of this companion submission before submission, consistent with each journal's policy on related or concurrent submissions. [Notification date to be recorded here once sent.]
...
\item \textbf{Evaluation and limitations} (Section~\ref{sec:limitations}): what the demonstration does and does not establish, stated as narrowly as Section~\ref{sec:coq}'s formal claims are.
\end{enumerate}
...
\toprule
\textbf{Failure mode} & \textbf{Architectural mechanism} & \textbf{Formal result} & \textbf{Demonstration} \\
\midrule
Unverified treated as accepted & Boundary specification, \textsf{Undefined} as a first-class output & Theorem~\ref{thm:default-freedom} (Default-Freedom) & Wirecard: confirmation-route failure (Section~\ref{sec:wirecard}) \\
\addlinespace
Proposer admits its own claim & Certificate-based admission against a process registry & Theorem~\ref{thm:admission} (Admission Separation) & Six adversarial fixtures in the companion development (Section~\ref{sec:coq}) \\
\addlinespace
Deprioritised exceptions vanish from record & Append-only Risk Assignment Register & Theorem~\ref{thm:residual} (Residual Preservation) & Continuous auditing: dashboard clearing (Section~\ref{sec:ca-case}) \\
\addlinespace
Partial evidence quietly carries an opinion & Composition: any unresolved material dependency blocks the opinion & Theorem~\ref{thm:main} / Corollary~\ref{cor:contra} & Both cases: the opinion remains inadmissible under partial evidence \\
\bottomrule
\end{tabular}
\caption{Failure modes, the mechanism each is answered by, the theorem proving that mechanism sound, and where it is demonstrated.}
\label{tab:dsrmap}
\end{table}
...
Verification residuals represent risk, and Boundary Discipline requires that every residual either be explicitly assigned to an identified owner or be flagged, visibly and durably, as orphaned -- the owner field is optional, not the escalation.
...
\label{thm:main}
For any audit opinion $O$, if $O$ is unqualified, then every material assertion on which $O$ depends is classified \textsf{Verified} under the declared boundary specification and admitted by a distinct, registered verifier identity holding verifier-level authority.
\end{theorem}
...
\label{thm:admission}
A proposed classification cannot be admitted by the same registered process identity that proposed it. Admission for audit consequence requires a distinct, registered identity holding verifier-level authority; whether two distinct registered identities are organisationally, economically, or substantively independent of one another is a governance question outside what this theorem addresses (see the qualification in Section~\ref{sec:pipeline}).
\end{theorem}
...
A direct objection is worth stating before it is raised by a reader: Equation~\ref{eq:mainform} defines an unqualified result to require verified, admitted dependencies, and the theorem below shows an unqualified result has verified, admitted dependencies -- is that not simply proving a definition true of itself? It would be, if the theorem's content were the definition. It is not. The theorem does not validate \emph{Unqualified}'s definition as a normative audit policy -- that this is the right way to define an admissible opinion is an argument this paper makes in prose (Sections~\ref{sec:principles}--\ref{sec:composition}), not something a proof assistant can adjudicate. What the theorem establishes, over and above the definition, is that the \emph{executable} pipeline described below conforms to that definition: that there is no path through the implementation, no combination of boundary specification, evidence context, registry, and certificates a caller can supply, that produces an \textsf{Unqualified} decision from a material dependency the same pipeline's own classification step left \textsf{Undefined} or otherwise unadmitted. An implementation could satisfy the abstract definition on paper while containing a bug that lets exactly this happen; the theorem is what rules that out for this one, not a restatement of what "unqualified" was defined to mean.
