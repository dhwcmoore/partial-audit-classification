...
\medskip
\noindent\textbf{Data availability statement:} This is a conceptual and methodological paper. It does not analyse a proprietary or empirical dataset. The illustrative boundary specification and classification pseudocode presented in the paper are original artefacts of the manuscript and are reproduced here in full. The admissibility, residual-preservation, and admission-separation theorems of Section~\ref{sec:formal} are additionally formalised and machine-checked, generally rather than only for the illustrative cases of Sections~\ref{sec:wirecard} and~\ref{sec:ca-case}, in a companion Coq/Rocq development and extracted OCaml classifier; \emph{[repository URL to be inserted before submission]}.

\medskip
\noindent\textbf{Note on a related submission:} A companion manuscript, \emph{A Discipline of Partial Audit Classification: On the Correct Treatment of Undefined Audit Assertions}, has been submitted to the \emph{International Journal on Software Tools for Technology Transfer}. Both manuscripts develop the same central invariant (Principle~\ref{prin:invariant}) and share the Coq/Rocq development described above. The present manuscript addresses an accounting information systems audience: the AIS pipeline architecture, the Verification Balance Sheet, the Risk Assignment Register, and the standards and inspection implications of Sections~\ref{sec:architecture}--\ref{sec:implications} are not treated in the companion submission. The companion submission instead addresses a formal-methods and tooling audience, and takes the mechanised admissibility theorem and its Coq/Rocq development as its central technical contribution rather than as a supporting artefact. We disclose the relationship so that editors and reviewers can evaluate the degree of overlap directly; in our own assessment the two manuscripts share an invariant and a proof artefact but not a contribution, and neither manuscript's findings depend on prior acceptance of the other.
...
The theorem-level core of the framework, presented in Section~\ref{sec:formal}, requires only the binary distinction between \textsf{Verified} and \textsf{Undefined} for affirmative purposes. Operationally, however, audit workflow benefits from a richer verdict domain, and the runtime verification literature shows how to enrich it without corrupting it. The classification function returns exactly one of six states.
...
\textsf{Escalation Required}: the claim exposes a defect in the boundary specification itself, such as an overlap of conflicting requirements, an orphaned residual, or a claim class for which no boundary was declared, and must be raised to the level at which specifications are governed.
...
The $\mathrm{state}$ function is not primitive: its value is computed mechanically from the declared boundary specification, so that verifiability is determined by declared boundaries and their application, not by discretion at the point of disappointment. The force of the statement lies in what it excludes. Given a fixed boundary specification and a fixed dependency set, there is no assignment of $\mathrm{state}$ and $\mathrm{admitted}$ satisfying Equation~\ref{eq:mainform} in which some material $a \in \mathrm{deps}(O)$ is $\textsf{Undefined}$ while $O$ is unqualified; the two are jointly inconsistent by construction. This restricted logic, evidence type, jurisdiction, materiality threshold, and control relationship mapped mechanically to a classification state, has been formalised and machine-checked in a companion Coq/Rocq development, quantified exactly as in Equation~\ref{eq:mainform} over abstract, unbounded assertion and opinion types rather than restricted to the illustrative cases of Sections~\ref{sec:wirecard} and~\ref{sec:ca-case}; \texttt{coqchk} confirms the development is closed under the global context with no \texttt{Admitted} and no \texttt{Axiom}. The proof is a direct consequence of the definition of admissibility as the per-dependency check itself, rather than an induction over the dependency set: there is no route to admissibility in the development that bypasses checking every material dependency, which is what it means for the exclusion described above to hold by construction rather than merely by convention. The same development proves Residual Preservation (Theorem~\ref{thm:residual}) and Admission Separation (Theorem~\ref{thm:admission}) in the same general form, and extracts to an executable OCaml classification component; the extraction step itself, like any Coq extraction, is trusted rather than independently machine-checked. See the companion repository, \emph{[repository URL to be inserted before submission]}, and its \texttt{NON\_CLAIMS.md} for the exact boundary of what this formal development does and does not establish.
